...
export default function BuildingsPage() {
    ...
    const { buildings, loading: buildingsLoading, error } = useBuildings(eventId);

    const loading = eventLoading || buildingsLoading;
        ...
        <div className="mx-auto min-h-screen max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            {loading ? (
                <div className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-500">建物情報を読み込み中...</p>
                </div>
            ) : error ? (
                <div className="rounded-lg border border-red-200 bg-red-50 px-6 py-4 text-center">
                    <p className="text-sm font-medium text-red-800">{error}</p>
                    <p className="mt-1 text-xs text-red-600">しばらくしてから再度お試しください</p>
                </div>
            ) : buildings.length === 0 ? (
